...
class HybridItemNeed(HybridNeed):
    def __init__(self, method, resource, type_=None):
        self.method = method
        self.type = type_ or resource.resource_name
    ...
    def __call__(self, item):
        ...
        return ItemNeed(self.method, self.resource.item_get_id(item), self.type)
    ...
    def make_expression(self):
        ...
        return self.resource._model_id_column.in_(ids)
...
class HybridRelationshipNeed(HybridItemNeed):
    ...
    def __call__(self, item):
        ...
        item_id = self.final_field.resource.item_get_id(item)
...
class HybridUserNeed(HybridRelationshipNeed):
    ...
    def __repr__(self):
        return '<HybridUserNeed {}>'.format(self.type)
